...
function PasswordInput({ className, ...props }: Omit<ComponentProps<typeof PasswordPrimitive.Input>, 'asChild'>) {
  return (
    <PasswordPrimitive.Input {...props} className={cn('pr-10', className)} asChild>
      <Input />
...
function PasswordToggle({ className, ...props }: PasswordToggleProps) {
  return (
    <PasswordPrimitive.Toggle
      className={cn(buttonVariants({ size: 'sq-xs', variant: 'ghost' }), className, 'absolute top-1/2 -translate-y-1/2 right-1')}
      {...props}
    >
