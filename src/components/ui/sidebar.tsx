...
function Sidebar({ className, children, ...props }: ComponentProps<'div'>) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          'bg-sidebar border flex flex-col size-full z-10 transition-[left,right,width] duration-200 ease-linear rounded',
          className,
        )}
        {...props}
      >
...
function SidebarContent({ className, ...props }: ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('min-h-0 flex flex-1 flex-col gap-2 overflow-hidden', className)}
      {...props}
    />
  );
}
